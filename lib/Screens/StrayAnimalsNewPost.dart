...
class StrayAnimalsNewPost extends StatefulWidget {
  const StrayAnimalsNewPost({Key? key}) : super(key: key);

  @override
  State<StrayAnimalsNewPost> createState() => _StrayAnimalsNewPostState();
}

class _StrayAnimalsNewPostState extends State<StrayAnimalsNewPost> {
  ...
  Future<void> postIt(BuildContext context) async {
    if (captionController.text.isEmpty ||
      ...
      if (user != null) {
        ...
        final postsCollection = FirebaseFirestore.instance.collection('strayanimals');
        ...
        for (int i = 0; i < selectedImages.length; i++) {
          ...
          final storagePath =
              'strayanimals/$userId/${DateTime.now().millisecondsSinceEpoch}_$i.jpg';
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => const SocialMediaPage(Si: 2,ci: 4)),
        );
